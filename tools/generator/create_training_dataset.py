#!/usr/bin/env python3
"""
Generate a fixed physical network and VNR dataset for training and evaluation.

Usage (after `conda activate virne`):
    python tools/generator/create_training_dataset.py \
        --output-dir datasets/generated/my_training_set \
        --num-vnrs 1000
"""
...
def generate_dataset(output_dir: Path, cfg: Config, seed: int) -> None:
    ...
    OmegaConf.save(snapshot, str(output_dir / "config_snapshot.yaml"))
    set_seed(seed)
    physical = PhysicalNetwork.from_setting(cfg.p_net_setting, seed=seed)
    physical.save_dataset(str(output_dir))
    ...
    simulator.save_dataset(str(output_dir))


def export_metadata(output_dir: Path, args: argparse.Namespace) -> None:
    metadata = {
    ...
    with (output_dir / "generation_meta.json").open("w") as f:
...
def build_config(args: argparse.Namespace) -> Config:
    ...
    cfg.dir_save_dataset = str(args.output_dir)
    cfg.save_dir = str(args.output_dir)
    cfg.summary_dir = str(args.output_dir)
    ...
    if 'save_dir' not in cfg.p_net_setting:
        cfg.p_net_setting['save_dir'] = cfg.p_net_setting.get('output', {}).get('save_dir', str(args.output_dir))
    if 'save_dir' not in cfg.v_sim_setting:
        cfg.v_sim_setting['save_dir'] = cfg.v_sim_setting.get('output', {}).get('save_dir', str(args.output_dir))

    cfg.p_net_setting["output"]["save_dir"] = str(args.output_dir)
    cfg.p_net_setting["save_dir"] = str(args.output_dir)
    ...
    v_setting["output"]["save_dir"] = str(args.output_dir)
    v_setting["save_dir"] = str(args.output_dir)
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a fixed VNR/physical dataset."
    )
    parser.add_argument("--output-dir", type=Path, default=Path("datasets/generated"), help="Directory to store the generated dataset.")
    parser.add_argument("--num-vnrs", type=int, default=1000, help="Number of virtual network requests to generate.")
...
def main(argv: List[str]) -> int:
    ...
    generate_dataset(output_dir, cfg, args.seed)
    export_metadata(output_dir, args)
