...
CONFIG_PATH="results/journal_suite/alpha_brain_seed2_k10_long.yaml"
PROFILE="alpha_brain_seed2_k10_long"
...
echo
echo "=== Brain seed=2 nominal k=10 latest eval ==="
conda run -n virne python -c "
...
matches = sorted(
    root.glob('journal_suite__alpha_zero_sfc__brain__nominal__seed2__eval__keval10__ckpt*/summary.csv'),
    key=lambda p: p.stat().st_mtime,
)

if not matches:
    print('missing nominal brain seed2 k=10 eval summary')
    raise SystemExit(0)

path = matches[-1]
row = next(csv.DictReader(path.open()))
print(f\"acceptance_rate={float(row['acceptance_rate']):.3f}\")
print(f\"clock_running_time={float(row['clock_running_time']):.3f}\")
