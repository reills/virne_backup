...
CONFIG_PATH="results/journal_suite/alpha_geant_k10_purecpp_seeds012.yaml"
PROFILE="alpha_geant_k10_purecpp_seeds012"
...
for seed in 0 1 2; do
  echo "  $ROOT_DIR/datasets/generated/journal/nominal/geant/seed_$seed/train"
done

echo "Using eval datasets:"
for seed in 0 1 2; do
  echo "  $ROOT_DIR/datasets/generated/journal/nominal/geant/seed_$seed/test"
done
...
if [[ " $STAGES " == *" eval "* ]]; then
echo
echo "=== AlphaZero Pure C++ Summary (geant, k=10, seeds 0/1/2) ==="
conda run -n virne python -c "
...
print(f'geant k=10 mean acceptance across seeds 0/1/2: {avg}')
"
fi
