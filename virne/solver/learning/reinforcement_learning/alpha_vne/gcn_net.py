...
class SequenceEncoder(nn.Module):
    """Smaller VNF encoder than the transformer stack in net.py."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        embedded = self.input_proj(x) + self.position_embed(positions)
        embedded = self.dropout(F.gelu(embedded))
        outputs = self.token_mixer(embedded)
        return self.norm(outputs)


class PNetEncoder(nn.Module):
    def __init__(self, input_dim: int, embedding_dim: int, gnn_layers: int, dropout: float):
        super().__init__()
        depth = max(1, int(gnn_layers))
        self.layers = nn.ModuleList()
        self.norms = nn.ModuleList()
        self.dropout = nn.Dropout(float(dropout))
        for layer_idx in range(depth):
            in_dim = input_dim if layer_idx == 0 else embedding_dim
            self.layers.append(GCNConv(in_dim, embedding_dim))
...
class SharedBackbone(nn.Module):
    """Smaller GCN + GRU backbone with the same AlphaZero interfaces as net.py."""
    ...
    def _summarize_history(
        ...
        batch_size, seq_len, hidden_dim = history_outputs.shape
    ...
    def _forward_backbone_core(
        ...
        del edge_attr
        graph_embedding = self.p_encoder(node_features.float(), edge_index)
        graph_embedding = self.node_norm(graph_embedding)
        graph_summary = global_mean_pool(graph_embedding, node_batch)

        history_tokens = self.history_embed(history_features)
        history_tokens = self.dropout(F.gelu(history_tokens))
