...
def select_action(
    ...
    if mask_actions and maskable_policy:
        candicate_action_probs = F.softmax(candicate_action_logits / softmax_temp, dim=-1)
        candicate_action_dist = Categorical(probs=candicate_action_probs)
    else:
        candicate_action_probs = F.softmax(action_logits / softmax_temp, dim=-1)
...
class BeamSearcher(Searcher):
    ...
    def find_solution(self, instance_env):
        ...
        while not sum(done_list):
            ...
            candicate_action_probs_list = get_action_distribution(
