...
## 5.1) Current Rollout Status

Baselines:
- baseline runs already exist for the main comparison set and should be treated as the fixed reference rows for seeds `0,1,2`
- benchmark targets called out in current tooling and result tables include `ppo`, `mcts`, `rinc`/meta baselines, and other journal rows already generated under `results/journal_suite`

AlphaZero-SFC pure C++:
- pure C++ parity regression on `wx100` seed `0` was fixed in the C++ core; the known request-2/step-5 divergence is gone
- validated pure-C++ operating point is the current canonical one for benchmark reruns:
  - `training.pure_cpp=true`
  - `training.use_cpp_mcts=true`
  - `training.computation_budget=96`
  - transformer backbone with `nn.embedding_dim=96`, `nn.hidden_dim=96`, `nn.num_gnn_layers=2`, `nn.n_heads=6`, `nn.transformer_layers=2`
  - train profile: `training.num_train_epochs=12`, `training.max_training_steps=1000`, `training.num_train_steps_per_epoch=128`, `training.min_buffer_size=128`, `training.max_empty_batches=1800`, `training.c_puct=1.4`, `training.num_workers=4`, `experiment.num_simulations=0`, `training.save_interval=2000`
- `wx100` seed `0` pure-C++ eval now completes cleanly with high acceptance and should be treated as the reference configuration for the remaining topology reruns

Immediate benchmark scope:
- reuse the existing paper seed set for `brain` and `geant`: seeds `0,1,2` are already present across the main baseline rows and AlphaZero rows
- compare new pure-C++ rows against the existing baseline summaries rather than regenerating the `brain` and `geant` baseline suite
- treat `wx100` as a sanity-check topology only unless a harder request distribution is introduced; current nominal `wx100` is saturated and weak as a headline comparison
- do not use nominal `wx500` as the headline scalability comparison because it is currently too easy and saturates near-perfect acceptance
- for `wx500`, use a tuned harder eval distribution generated by `tools/build_wx500_hard_compare_config.py`; target a mid-band acceptance regime and avoid both saturation and collapse before launching the full benchmark matrix
...
1. Use `tools/run_alpha_multitopo_purecpp_final.sh` as the canonical launcher for `geant` and `brain` seeds `0,1,2`, and only keep `wx100` as a secondary sanity row unless a harder dataset is defined.
2. Use `tools/run_wx500_hard_compare_alpha_zero_sfc.sh` and the companion baseline scripts for the tuned `wx500` compare cell, with difficulty knobs explicitly set and logged.
3. Aggregate latest-per-cell results and compare only against matched baseline rows already present in the suite outputs.
